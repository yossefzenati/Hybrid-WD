...
!          b% s1% job% warn_run_star_extras = .false.
      ...
      integer function extras_binary_finish_step(binary_id)
         ...
         if (ierr /= 0) then ! failure in  binary_ptr
          ...
          if (b% s1% log_surface_temperature < 5.25d0 .AND. b% s1% log_surface_luminosity < 2.1d0) then
	         extras_binary_finish_step = terminate
         end if
          !if (extras_finish_step == terminate) b% s1% termination_code = t_extras_finish_step
